...
<div class="max-w-2xl">
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 md:p-8">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h3 class="text-xl font-bold text-slate-900 dark:text-white">Edit Rekening</h3>
                <p class="text-sm text-slate-500">Update data rekening {{ $rekening->nama_bank }}</p>
            </div>
        </div>

        <form action="{{ route('rekening.update', $rekening->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="mb-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                    <p class="text-xs text-slate-500 mb-2">QR Code saat ini:</p>
                    <img src="{{ asset('storage/' . $rekening->qr_code) }}" alt="Current QR Code" class="w-32 h-32 object-contain border border-slate-200 dark:border-slate-700 rounded-lg bg-white">
                </div>
                @endif
                ...
                <div class="flex items-center gap-4">
                    <label for="qr_code" class="flex-1 flex flex-col items-center justify-center px-4 py-6 bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-lg cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-all">
                        ...
                        <span class="text-sm text-slate-600 dark:text-slate-400 font-medium">Klik untuk upload QR Code baru</span>
                ...
                @error('qr_code')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-slate-500">Upload gambar baru untuk mengganti QR code (opsional)</p>
            </div>
