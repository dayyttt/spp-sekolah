...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div class="p-4 md:p-6 border-b border-slate-200 dark:border-slate-800">
            <h3 class="text-base md:text-lg font-bold text-slate-900 dark:text-white">Form Data Kelas</h3>
            <p class="text-xs md:text-sm text-slate-500 mt-1">Lengkapi form di bawah untuk menambahkan kelas baru</p>
        </div>

        <form action="{{ route('kelas.store') }}" method="POST" class="p-4 md:p-6 space-y-4 md:space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="nama_kelas" value="{{ old('nama_kelas') }}" placeholder="Contoh: X IPA 1" class="w-full px-3 md:px-4 py-2 md:py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all @error('nama_kelas') border-red-500 @enderror" required>
                @error('nama_kelas')
            ...
            <div>
                ...
                <input type="text" name="jurusan" value="{{ old('jurusan') }}" placeholder="Contoh: IPA, IPS" class="w-full px-3 md:px-4 py-2 md:py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary transition-all">
                <p class="mt-1 text-xs text-slate-500">Opsional - Kosongkan jika tidak ada jurusan</p>
            </div>
