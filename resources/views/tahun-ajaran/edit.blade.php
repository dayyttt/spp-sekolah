...
<div class="max-w-2xl">
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 md:p-8">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h3 class="text-xl font-bold text-slate-900 dark:text-white">Edit Tahun Ajaran</h3>
                <p class="text-sm text-slate-500">Update data tahun ajaran {{ $tahunAjaran->tahun }}</p>
            </div>
        </div>

        <form action="{{ route('tahun-ajaran.update', $tahunAjaran->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input 
                        ...
                        required
                    >
                    <input type="hidden" name="nominal_spp" id="nominal_spp" value="{{ old('nominal_spp', $tahunAjaran->nominal_spp) }}">
                </div>
                @error('nominal_spp')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set nilai awal dari hidden input (untuk edit form)
    if (nominalHidden.value) {
        ...
        nominalHidden.value = Math.round(parseFloat(nominalHidden.value));
    }
